...
ENABLE_ARGO_WORKFLOW=${ENABLE_ARGO_WORKFLOW:-$ENABLE_K3S}
...
EOF
  ...
  if [ "$ENABLE_NVIDIA_GPU" == "true" ]; then
    retry curl -sfL https://rancher-mirror.rancher.cn/k3s/k3s-install.sh | INSTALL_K3S_MIRROR=cn INSTALL_K3S_VERSION=v1.30.4+k3s1 sh -s - --disable=traefik --default-runtime=nvidia --bind-address=$IP_ADDRESS
  else
    retry curl -sfL https://rancher-mirror.rancher.cn/k3s/k3s-install.sh | INSTALL_K3S_MIRROR=cn INSTALL_K3S_VERSION=v1.30.4+k3s1 sh -s - --disable=traefik --bind-address=$IP_ADDRESS
  fi
